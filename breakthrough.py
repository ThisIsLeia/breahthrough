...
class StrategyBase:
    ...
    def _getdata(self, filename='D:/Course/python/交易策略/breahthrough/TXFB9I020.csv'):
        """讀取資料"""
    ...
    def _strategy(self, tidy_data):
        ...
        for date_ in date_list:
            ...
            # 取得當日九點前股價資料 判斷行情
            data_before9 = [int(i[3]) for i in day_data if int(i[1]) < 90000000000]

            # 取得當日九點到十一點資料
            data_before11 = [i for i in day_data if int(i[1]) >= 90000000000 and int(i[1]) <= 110000000000]
            ...
            for i in range(len(data_before11)):
                # 當前時間
                time = data_before11[i][1]
                price = int(data_before11[i][3])
                # 建倉判斷
                if index == 0:
                    # 多單進場
                    if price > int(ceil):
